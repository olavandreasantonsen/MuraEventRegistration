				...
				<cfswitch expression="#URL.PerformAction#">
					<cfcase value="AddUser">
						...
						<cfif not isValid("email", FORM.Email)>
							...
							<cflocation url="/index.cfm?EventRegistrationaction=admin:users.adduser&PerformAction=AddUser" addtoken="false">
						</cfif>
						...
						<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:users&UserAction=AddedUser&SiteID=#rc.$.siteConfig('siteID')#&Successful=true" addtoken="false">
					</cfcase>
				</cfswitch>
			</cfif>
		</cfif>
				...
				<cfswitch expression="#URL.PerformAction#">
					<cfcase value="ChangePassword">
						...
						<cfif FORM.Password NEQ FORM.VerifyPassword>
							...
							<cflocation url="/plugins/EventRegistration/index.cfm?EventRegistrationaction=admin:users.updateuser&PerformAction=ChangePassword&RecNo=#FORM.RecNo#" addtoken="false">
						<cfelse>
							<!--- Initiates the User Bean --->
							<cfset NewUser = #Application.userManager.readByUsername(form.Username, rc.$.siteConfig('siteID'))#>
							<CFSET NewUser.setPassword(FORM.Password)>
							<cfset NewUser.save()>
							<cflocation url="/plugins/EventRegistration/index.cfm?EventRegistrationaction=admin:users&Successful=true&UserAction=ChangePassword" addtoken="false">
						</cfif>
					</cfcase>
					<cfcase value="Edit">
						<cflock timeout="60" scope="SESSION" type="Exclusive">
						...
						<cfif UpdateInfo.Fname EQ 1 OR UpdateInfo.Lname EQ 1 OR UpdateInfo.UserName EQ 1 OR UpdateInfo.Email EQ 1 OR UpdateInfo.JobTitle EQ 1 OR UpdateInfo.mobilePhone EQ 1 OR UpdateInfo.Active EQ 1>
							...
							<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:users&UserAction=UpdatedUser&SiteID=#rc.$.siteConfig('siteID')#&Successful=true" addtoken="false">
						</cfif>
						...
						<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:users" addtoken="false">
					</cfcase>
				</cfswitch>
			</cfif>
		</cfif>
		...
		<cfif isDefined("URL.PerformAction")>
			<cfswitch expression="#URL.PerformAction#">
				...
				<cfcase value="Delete">
					...
					<cflocation url="?#HTMLEditFormat(rc.pc.getPackage())#action=admin:users.default&Successful=true&UserAction=DeActivateAccount" addtoken="false">
				</cfcase>
			</cfswitch>
		</cfif>
